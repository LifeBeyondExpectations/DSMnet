...
from util_conv import net_init, conv2d_bn, deconv4x4_bn, mycat
...
class dispnet(nn.Module):
    def __init__(self, maxdisparity=192):
        ...
        # 卷积层
        self.conv1 = conv2d_bn(6, 64, kernel_size=7, stride=2, bn=flag_bn, activefun=activefun_default)
        self.conv2 = conv2d_bn(64, 128, kernel_size=5, stride=2, bn=flag_bn, activefun=activefun_default)
        self.conv3a = conv2d_bn(128, 256, kernel_size=5, stride=2, bn=flag_bn, activefun=activefun_default)
...
#            m.weight.data = m.weight.data*1e-1
    ...
    def forward(self, imL, imR, mode="train"):
        ...
        iconv2 = self.iconv2(mycat([deconv2, self.upsample(pr3), conv2, ], dim=1))
        pr2 = self.pr2(iconv2)
        ...
        iconv1 = self.iconv1(mycat([deconv1, self.upsample(pr2), conv1, ], dim=1))
        pr1 = self.pr1(iconv1)
