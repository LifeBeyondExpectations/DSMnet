...
flag_bn = False # 是否使用正则化层
flag_bias = True # 将卷积层是否使用偏移
activefun_default = nn.ReLU(inplace=True) # 默认的激活函数
...
def conv2d_bn(in_planes, out_planes, kernel_size=3, stride=1, bn=flag_bn, activefun=activefun_default):
    "convolution with padding, bn and activefun"
    assert kernel_size % 2 == 1
    conv2d = Conv2d(in_planes, out_planes, kernel_size, stride, padding=kernel_size//2, bias=flag_bias)
...
def deconv3x3_bn(in_planes, out_planes, bn=flag_bn, activefun=activefun_default):
    "3x3 deconvolution with padding, bn and activefun"
    deconv2d = ConvTranspose2d(in_planes, out_planes, 3, 2, padding=1, output_padding=1, bias=flag_bias)
...
def deconv4x4_bn(in_planes, out_planes, bn=flag_bn, activefun=activefun_default):
    "4x4 deconvolution with padding, bn and activefun"
    deconv2d = ConvTranspose2d(in_planes, out_planes, 4, 2, padding=1, output_padding=0, bias=flag_bias)
...
def deconv8x8_bn(in_planes, out_planes, bn=flag_bn, activefun=activefun_default):
    "8x8 deconvolution with padding, bn and activefun"
    deconv2d = ConvTranspose2d(in_planes, out_planes, 8, 4, padding=2, output_padding=0, bias=flag_bias)
...
def conv3d_bn(in_planes, out_planes, kernel_size=3, stride=1, bn=flag_bn, activefun=activefun_default):
    "3x3x3 convolution with padding"
    conv3d = Conv3d(in_planes, out_planes, kernel_size, stride, padding=1, bias=flag_bias)
...
def conv3x3x3_bn(in_planes, out_planes, stride=1, bn=flag_bn, activefun=activefun_default):
    "3x3x3 convolution with padding"
    conv3d = Conv3d(in_planes, out_planes, 3, stride, padding=1, bias=flag_bias)
...
def deconv3x3x3_bn(in_planes, out_planes, bn=flag_bn, activefun=activefun_default):
    "3x3x3 deconvolution with padding"
    deconv3d = ConvTranspose3d(in_planes, out_planes, 3, 2, padding=1, output_padding=1, bias=flag_bias)
    
    if(not bn and not activefun): 
        return deconv3d
        
    layers = []
    layers.append(deconv3d)
    if bn: layers.append(nn.BatchNorm3d(out_planes))
    if activefun: layers.append(activefun)
...
def deconv4x4x4_bn(in_planes, out_planes, bn=flag_bn, activefun=activefun_default):
    "4x4x4 deconvolution with padding"
    deconv3d = ConvTranspose3d(in_planes, out_planes, 4, 2, padding=1, output_padding=0, bias=flag_bias)
